...
def script_generator(subject, video_length, creativity):
   ...
   body_template = ChatPromptTemplate.from_messages(
       [
           ("human",
            """"你是一位25岁的短视频博主，会定期在抖音、快手、TikTok上发布优质的视频内容，以轻松幽默的风格和犀利的洞察视角著称，获得全网千万粉丝。
            根据以下标题和相关信息，为你的频道写一个视频口播脚本。
            视频标题：{title}，视频时长：{duration}分钟，生成的脚本文本长度需要考虑视频时长要求，一般1分钟的口播脚本大概包含100-250个字。
            开头抓住观众眼球，快速切入主题；中间段落占据大量篇幅，提供知识点，体现你专业的分析和洞察能力；结尾处以幽默诙谐的方式收尾，让观众记住你的风格。
            脚本格式按照【开头、中间段落、结尾】分隔。
            注意你的粉丝群体是18-30岁的都市独立女性，用吸引她们的方式进行表达。
            脚本内容可以结合以下维基百科搜索出的信息，但仅作参考，只结合相关内容即可，对不相关的进行忽略：
            '''{wikipedia_search}'''""")
       ]
   )

   model = ChatOpenAI(openai_api_key = "sk-yqQFMDPgJ5fJLEwM8e3c788863Cb47D084F31f5a8490F4E6",
